...
```
docker run -ti -v $(pwd):/workspace caffe:cpu caffe train --solver=example_solver.prototxt
```
...
```
docker run -ti -v $(pwd):/workspace -u $(id -u):$(id -g) caffe:cpu caffe train --solver=example_solver.prototxt
```
